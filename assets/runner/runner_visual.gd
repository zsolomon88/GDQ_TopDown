...
##
## You can change the playing animation with the [member animation_name] property, and
...
@export_range(-180.0, 180.0, 0.001, "radians_as_degrees") var angle := 0.0:
	set = set_angle

## Controls the animation that's currently playing. Use one of the values from the [enum Animations] enum.
@export var animation_name: Animations = Animations.IDLE:
